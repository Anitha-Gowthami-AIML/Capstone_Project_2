...
def inject_theme():
    ...
    wallpaper = _wallpaper_svg_data_uri()
    st.markdown(
        f"""
        <style>
        /* ---- App-wide base: rich purple / lavender / sky-blue wallpaper ----
           NOTE: layered directly on .stApp's own `background` (gradient +
           SVG image in one declaration) instead of a ::before/filter/blur
        ...
        .stApp {{
            background:
                linear-gradient(180deg, rgba(44,33,86,0.78) 0%, rgba(70,58,122,0.72) 35%,
                                         rgba(93,111,163,0.62) 70%, rgba(143,184,214,0.5) 100%),
                url("{wallpaper}");
